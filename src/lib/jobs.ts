...
// Fetch from JoBoard (free RSS-based)
async function fetchJoboardJobs(): Promise<RealJob[]> {
  try {
    const res = await fetch(
      "https://jooble.org/api/",
      {
        ...
        body: JSON.stringify({ keywords: "developer", location: "" }),
        next: { revalidate: 600 },
      }
    );
    // Jooble often requires API key, so this might fail - that's ok
    if (!res.ok) return [];
    const data = await res.json();
    return (data.jobs || []).slice(0, 10).map((job: any, i: number) => ({
...
// Research-based curated listings from major companies
...
export async function fetchAllJobs(query?: string): Promise<RealJob[]> {
  ...
  // If there's a search query, also filter curated results
  if (query) {
...
export function filterJobs(
  ...
  return jobs.filter((job) => {
    ...
    if (filters.mode && filters.mode !== "all") {
      if (!job.mode.toLowerCase().includes(filters.mode.toLowerCase()))
        return false;
    }
